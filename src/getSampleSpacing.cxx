...
int main(int argc, char **argv)
{
	...
	if(argc == 2)
		...
		cerr << endl
			<< "getSampleSpacing version 1.0.0" << endl
			<< endl
			<< "This command gets the sample spacing from a specified DICOM file." << endl
			<< endl
			<< "Usage: getSampleSpacing <DICOM file> <optional: Log file>" << endl
			<< "\tExample: GetSampleSpacing IM-0001-0001-0001.dcm" << endl
			<< endl
			<< "If you also want to check values necessary for calculating the sample spacing, execute like this:" << endl
			<< "\tGetSampleSpacing IM-0001-0001-0001.dcm OutputFileNameYouWant.txt" << endl
			<< endl
	...
	/* --- Read DICOM Fields --- */
	double d1 = readDICOMfield(dcmdata, "0018", "0095", &log); // (0018, 0095)
	double d2 = readDICOMfield(dcmdata, "0028", "0010", &log); // (0028, 0010)
	double dans = 1.0 / (d1 * d2);
	
	ostringstream sout;
	sout << fixed << setprecision(12) << dans;
	string dstr = sout.str();
	log += "\nSampleSpacing: " + dstr + "\n\tSampleSpacing = 1 / ((0018,0095) * (0028,0010)))";
	
	/* --- Output Values Necessary for Calculating the Sample Spacing --- */
	if(argc == 3){
		ofstream ofs(ofn);
		ofs << log;
	}
	
	/* --- OutPut the Sample Spacing --- */
